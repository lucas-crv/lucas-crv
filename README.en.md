# Lucas Carvalho Santana
### Power Platform & Full-Stack Developer 🚀

<div align="right">
  <a href="README.md"><img src="https://img.shields.io/badge/Português-grey?style=flat-square" alt="Versão em PT-BR"></a>
  <a href="README.en.md"><img src="https://img.shields.io/badge/English-blue?style=flat-square" alt="English Version"></a>
</div>

---

### 🚀 About Me
I bridge the gap between **low-code agility** and **pro-code robustness**. With a strong background in the Microsoft Power Platform and a deep dive into Full-Stack development (React/Node.js), I specialize in building secure, scalable, and user-centric business solutions.

* **Expertise:** Solving complex logic with Power Apps/Automate and building high-security web portals with modern stacks.
* **Security Focused:** Experienced in handling sensitive data (PII) using AES-256 encryption and JWT authentication.
* **Business Impact:** From orchestrating US-based logistics ecosystems to streamlining professional recruitment funnels.

---

### 🛠 Technical Stack

**Low-Code & Automation**
![Power Apps](https://img.shields.io/badge/-Power_Apps-742774?style=flat&logo=microsoft-power-apps&logoColor=white)
![Power Automate](https://img.shields.io/badge/-Power_Automate-0066FF?style=flat&logo=microsoft-power-automate&logoColor=white)
![Dataverse](https://img.shields.io/badge/-Dataverse-0078D4?style=flat&logo=microsoft-dataverse&logoColor=white)
![SharePoint](https://img.shields.io/badge/-SharePoint-0078D4?style=flat&logo=microsoft-sharepoint&logoColor=white)

**Pro-Code (Full-Stack)**
![React](https://img.shields.io/badge/-React-61DAFB?style=flat&logo=react&logoColor=black)
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat&logo=node.js&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)

**Security & DevTools**
![Cybersecurity](https://img.shields.io/badge/-AES--256_Encryption-red?style=flat)
![JWT](https://img.shields.io/badge/-JWT-black?style=flat&logo=json-web-tokens)
![Git](https://img.shields.io/badge/-Git-F05032?style=flat&logo=git&logoColor=white)
![Railway](https://img.shields.io/badge/-Railway-0B0D0E?style=flat&logo=railway&logoColor=white)

---

### 📂 Featured Projects

<table>
  <tr>
    <td width="50%" valign="top">
      <h4>📦 US Logistics Ecosystem</h4>
      <p align="justify">
        <b>Purpose:</b> Orchestrated end-to-end shipping cycles for a US-based enterprise. Developed a 6-app ecosystem to centralize logistics data and automate dispatching.
      </p>
      <code>PowerApps</code> <code>Automate</code> <code>Architecture</code>
      <br><br>
      <a href="https://github.com/lucas-crv">
        <img src="https://img.shields.io/badge/View_Project-001242?style=for-the-badge&logo=github&logoColor=white" />
      </a>
    </td>
    <td width="50%" valign="top">
      <h4>📑 Driver Recruitment Portal</h4>
      <p align="justify">
        <b>Purpose:</b> Full-stack platform for driver hiring. Features PII encryption (AES-256), digital signatures, and automated official PDF generation.
      </p>
      <code>React</code> <code>Node.js</code> <code>PostgreSQL</code> <code>Auth</code>
      <br><br>
      <a href="https://github.com/lucas-crv">
        <img src="https://img.shields.io/badge/View_Project-001242?style=for-the-badge&logo=github&logoColor=white" />
      </a>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h4>📶 Applus Offline Entry</h4>
      <p align="justify">
        <b>Purpose:</b> Built for field teams in remote areas. Ensures 100% data integrity with offline-first capabilities and automated background synchronization.
      </p>
      <code>Offline-Sync</code> <code>PowerApps</code> <code>Dataverse</code>
      <br><br>
      <a href="https://github.com/lucas-crv">
        <img src="https://img.shields.io/badge/View_Project-001242?style=for-the-badge&logo=github&logoColor=white" />
      </a>
    </td>
    <td width="50%" valign="top">
      <h4>📍 Google Maps Connector</h4>
      <p align="justify">
        <b>Purpose:</b> API bridge for geolocation. Created a custom connector to inject real-time routing and maps data directly into business workflows.
      </p>
      <code>Custom Connector</code> <code>JSON/API</code> <code>Maps</code>
      <br><br>
      <a href="https://github.com/lucas-crv">
        <img src="https://img.shields.io/badge/View_Project-001242?style=for-the-badge&logo=github&logoColor=white" />
      </a>
    </td>
  </tr>
</table>

---

### 🎮 Beyond Code
When I'm not architecting solutions, you'll find me exploring new **AI trends**, gaming, or training my **puppy**. I'm a tech enthusiast who loves keeping up with the latest hardware and software innovations.

### 📫 Let's Connect!
* [LinkedIn](https://www.linkedin.com/in/lucas-carvalho-santana/)
* [GitHub](https://github.com/lucas-crv)
